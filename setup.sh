...
confirm_installation() {
    ...
    echo "  • NTREE Autonomous Mode (Claude SDK + API)"
...
install_python_tools() {
    ...
    pip install "anthropic>=0.40.0" "claude-code-sdk>=0.0.25" "python-dotenv>=1.0.0" "schedule>=1.2.0" "colorlog>=6.7.0" "python-json-logger>=2.0.0" "tenacity>=8.2.0"
...
install_autonomous_mode() {
    log_step "Installing NTREE Autonomous Mode..."
    ...
    # Check if source and target are the same
    if [[ "$SOURCE_PATH" == "$TARGET_PATH" ]]; then
        log_info "Autonomous mode already in target location, updating in place..."
    else
        # Copy to temporary location first to avoid conflicts
        TEMP_DIR=$(mktemp -d)
        log_info "Copying autonomous mode via temp directory..."
    ...
    # Copy config example (only if it doesn't exist)
    if [[ ! -f ~/ntree/config.json ]]; then
        cp ~/ntree/ntree-autonomous/config.example.json ~/ntree/config.json
        log_info "Created config.json from example"
    else
        log_warning "config.json already exists, skipping to preserve user configuration"
    fi
...
show_next_steps() {
    ...
    echo ""
    echo "For Autonomous API Mode, you also need:"
    ...
    echo ""
    echo "3. Autonomous API Mode:"
    echo "   ${GREEN}export ANTHROPIC_API_KEY='sk-ant-...'${NC}"
    echo "   ${GREEN}python ~/ntree/ntree-autonomous/ntree_agent.py --scope ~/ntree/templates/scope_example.txt${NC}"
    echo ""
    echo "4. Autonomous SDK Mode:"
