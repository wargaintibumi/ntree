...
async def _test_with_nuclei(host: str, vuln_id: str, port: int) -> dict:
    ...
    try:
        ...
        command = f"nuclei -u {target} -id {vuln_id} -silent"
...
async def _test_smb_creds(host: str, username: str, password: str, hash_value: str) -> dict:
    ...
    try:
        ...
        if hash_value:
            command = f"crackmapexec smb {host} -u '{username}' -H '{hash_value}'"
        else:
            command = f"crackmapexec smb {host} -u '{username}' -p '{password}'"
...
async def _test_ssh_creds(host: str, username: str, password: str) -> dict:
    ...
    try:
        ...
        command = f"sshpass -p '{password}' ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 {username}@{host} 'echo SUCCESS'"
        ...
        if returncode == 0 and 'SUCCESS' in stdout:
            ...
            sudo_check = f"sshpass -p '{password}' ssh -o StrictHostKeyChecking=no {username}@{host} 'sudo -n true'"
...
async def _test_ftp_creds(host: str, username: str, password: str) -> dict:
    ...
    try:
        ...
        command = f"curl -u '{username}:{password}' ftp://{host}/ --max-time 10"
...
async def _test_rdp_creds(host: str, username: str, password: str) -> dict:
    ...
    try:
        command = f"crackmapexec rdp {host} -u '{username}' -p '{password}'"
