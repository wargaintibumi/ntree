...
Tests all 6 MCP servers to verify functionality
"""
...
"""
        ...
        logger.info("Test 1: Initialize engagement")
        result = await init_engagement(str(scope_file), "")
        assert result["status"] == "success", "Engagement initialization failed"
        engagement_id = result["engagement_id"]
        logger.info(f"[PASS] Engagement created: {engagement_id}")
...
async def test_scan_server(engagement_id):
    """Test scan.py server"""
    logger.info("=" * 60)
...
async def test_enum_server(engagement_id):
    """Test enum.py server"""
    logger.info("=" * 60)
...
async def test_vuln_server(engagement_id):
    """Test vuln.py server"""
    logger.info("=" * 60)
...
async def test_post_server(engagement_id):
    """Test post.py server"""
    logger.info("=" * 60)
    logger.info("Testing POST Server")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("Test 1: Function signature validation")
        logger.info("[PASS] analyze_trust function exists")
        logger.info("[PASS] extract_secrets function exists")
        logger.info("[PASS] map_privileges function exists")

        # Test 2: Extract secrets requires approval
        logger.info("Test 2: Extract secrets approval check")
        ...
        assert result["status"] == "error", "Should require approval"
        assert "APPROVAL REQUIRED" in result.get("error", ""), "Missing approval check"
        logger.info("[PASS] Approval requirement enforced")

        logger.info("[PASS] POST Server: ALL TESTS PASSED")
        return True

    except Exception as e:
        logger.error(f"[FAIL] POST Server: FAILED - {e}")
        import traceback
        traceback.print_exc()
        return False


async def test_report_server(engagement_id):
    ...
    try:
        ...
        result = await score_risk(engagement_id)
        ...
        result = await generate_report(engagement_id, "executive", "markdown")
        ...
        result = await generate_report(engagement_id, "technical", "html")
        assert result["status"] == "success", "Report generation failed"
        ...
        result = await generate_report(engagement_id, "comprehensive", "markdown")
        assert result["status"] == "success", "Report generation failed"
...
async def test_integration():
    ...
    try:
        ...
        logger.info("Step 1: Initialize engagement")
        result = await init_engagement(str(scope_file), "")
        engagement_id = result["engagement_id"]
        logger.info(f"[PASS] Engagement: {engagement_id}")
        ...
        result = await generate_report(engagement_id, "comprehensive", "html")
...
async def run_all_tests():
    ...
    # Test each server
    results["scope"], engagement_id = await test_scope_server()

    if engagement_id:
        results["scan"] = await test_scan_server(engagement_id)
        results["enum"] = await test_enum_server(engagement_id)
        results["vuln"] = await test_vuln_server(engagement_id)
        results["post"] = await test_post_server(engagement_id)
        results["report"] = await test_report_server(engagement_id)
        results["integration"] = await test_integration()
    else:
        logger.error("Cannot run other tests without engagement_id")
        results["scan"] = False
        results["enum"] = False
        results["vuln"] = False
        results["post"] = False
